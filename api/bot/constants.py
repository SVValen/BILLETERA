AYUDA = (
    ...
    "  `/inversiones` — recomendaciones RV + carry trade + opciones RF\n"
    "  `/portafolio` — distribución de activos RV y P&L\n"
    "  `/precios` — cotizaciones en tiempo real\n"
    "  `/como_funciona` — cómo se calculan las señales\n\n"

    "💼 *Renta Fija y Liquidez:*\n"
    "  `/plan_renta` — asistente: capital → objetivo → broker → planes sugeridos\n"
    "  `/opciones_rf` — análisis de cauciones, letras y bonos disponibles\n"
